...
function initDOMCompare() {
	browser.tabs.query({ active: true, currentWindow: true }, async function (tabs) {
		...
		var renderedObjURL = tabHTMLBlob.payload;
		var renderedObjKey = RENDERED_OBJURL_KEY_PREFIX + tabs[0].id;
		var doctypeKey = DOCTYPE_KEY_PREFIX + tabs[0].id;
		var uaKey = UA_KEY_PREFIX + tabs[0].id;
